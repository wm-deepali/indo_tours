{{-- resources/views/admin/tourpackage/index.blade.php --}}
...
<div class="main-section">
    ...
    .badge-cat { display: inline-block; padding: 3px 10px; border-radius: 999px; font-size: 11.5px; font-weight: 600; background: var(--accent-light); color: var(--accent); }
    ...
    <div class="app-content content container-fluid">
        <div class="cat-page">
            ...
            <div class="cat-page-header">
                ...
                <a href="{{ route('admin.tourpackages.create') }}" class="btn-primary-dash">
                    <i class="fa fa-plus"></i> Add Tour Package
                </a>
            </div>
            ...
            <div class="cat-card">
                <table class="dest-table">
                    ...
                    <tbody>
                        @forelse($tourPackages as $tp)
                            <tr>
                                <td>
                                    @if($tp->main_image)
                                        <img src="{{ asset($tp->main_image) }}" class="dest-thumb" alt="{{ $tp->name }}">
                                    @else
                                ...
                                <td>
                                    <div class="dest-name">{{ $tp->name }}</div>
                                    <div class="dest-location">{{ Str::limit($tp->banner_intro, 50) }}</div>
                                </td>
                                <td>
                                    @if($tp->subCategory)
                                        <span class="badge-cat">{{ $tp->subCategory->name }}</span>
                                    @else
                                        —
                                    @endif
                                </td>
                                <td>{{ $tp->duration_text ?? '—' }}</td>
                                <td>
                                    @if($tp->price)
                                        ₹{{ number_format($tp->price) }}
                                        @if($tp->old_price)
                                            <br><small style="color:var(--text-hint);text-decoration:line-through;">₹{{ number_format($tp->old_price) }}</small>
                                        @endif
                                    @else
                                        —
                                    @endif
                                ...
                                <td>
                                    <span class="badge-status badge-{{ $tp->status }}">
                                        {{ ucfirst($tp->status) }}
                                    </span>
                                </td>
                                <td>
                                    <div class="row-actions" style="justify-content:flex-end">
                                        <a href="{{ route('admin.tourpackages.edit', $tp) }}">
                                            <i class="fa fa-pen"></i> Edit
                                        </a>
                                        <form action="{{ route('admin.tourpackages.destroy', $tp) }}" method="POST"
                        ...
                        @empty
                            <tr>
                                <td colspan="7">
                                    <div class="empty-state">No tour packages added yet.</div>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>

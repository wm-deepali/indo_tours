{{-- resources/views/admin/attraction/index.blade.php --}}
    ...
    <div class="app-content content container-fluid">
        <div class="cat-page">
            ...
            <div class="cat-page-header">
                ...
                <a href="{{ route('admin.attractions.create') }}" class="btn-primary-dash">
                    ...
                    <tbody>
                        @forelse($attractions as $attraction)
                            <tr>
                                <td>
                                    @if($attraction->image)
                                        <img src="{{ asset('storage/' . $attraction->image) }}" class="dest-thumb" alt="{{ $attraction->name }}">
                                    @else
                                ...
                                <td>
                                    <div class="dest-name">{{ $attraction->name }}</div>
                                    <div class="dest-location">{{ $attraction->short_description }}</div>
                                </td>
                                <td>
                                ...
                                <td>
                                    <span class="badge-status badge-{{ $attraction->status }}">
                                        {{ ucfirst($attraction->status) }}
                                    </span>
                                </td>
                                <td>{{ $attraction->is_featured ? 'Yes' : '—' }}</td>
                                <td>
                        ...
                        @empty
                            <tr>
                                <td colspan="6">
                                    <div class="empty-state">No attractions added yet.</div>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>

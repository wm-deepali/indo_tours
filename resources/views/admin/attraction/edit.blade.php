{{-- resources/views/admin/attraction/edit.blade.php --}}
                ...
                <form action="{{ route('admin.attractions.update', $attraction) }}" method="POST"
                    ...
                    <div class="cat-tab-panel active" data-panel="general">
                        ...
                        <div class="form-field">
                            ...
                            @if($attraction->image)
                                <img src="{{ asset('storage/' . $attraction->image) }}" class="current-img-preview"
                                    alt="{{ $attraction->name }}">
                            @endif
                        ...
                        <div class="form-field">
                            ...
                            <textarea id="short_description" name="short_description" rows="2"
                                class="form-control-styled @error('short_description') is-invalid @enderror">{{ old('short_description', $attraction->short_description) }}</textarea>
                        ...
                        <div class="form-field">
                            ...
                            <textarea id="description" name="description" rows="5"
                                class="form-control-styled @error('description') is-invalid @enderror">{{ old('description', $attraction->description) }}</textarea>
                            @error('description')
                            <div class="form-error">{{ $message }}</div>@enderror
                        </div>
                        ...
                        <div class="form-field">
                            ...
                            @if($attraction->about_image)
                                <img src="{{ asset('storage/' . $attraction->about_image) }}" class="current-img-preview"
                                    alt="">
                            @endif
                            ...
                            <div class="hint">Leave blank to keep the current About image (falls back to the Image above
                        ...
                        <div class="form-field">
                            ...
                            <textarea id="about_content" name="about_content" rows="8"
                                class="ckeditor form-control-styled @error('about_content') is-invalid @enderror">{{ old('about_content', $attraction->about_content) }}</textarea>
                        ...
                        <div class="form-row">
                            <div class="form-field">
                                ...
                                <input type="number" id="rating" name="rating" step="0.1" min="0" max="5"
                                    ...
                                    value="{{ old('rating', $attraction->rating) }}">
                                @error('rating')
                                <div class="form-error">{{ $message }}</div>@enderror
                            </div>
                            <div class="form-field">
                                ...
                                <input type="number" id="review_count" name="review_count" min="0"
                                    ...
                                    value="{{ old('review_count', $attraction->review_count) }}">
                                @error('review_count')
                                <div class="form-error">{{ $message }}</div>@enderror
                            </div>
                        </div>
                        ...
                        <div class="form-field">
                            ...
                            <select id="status" name="status" class="form-control-styled">
                                <option value="draft" {{ old('status', $attraction->status) == 'draft' ? 'selected' : '' }}>Draft</option>
                                <option value="published" {{ old('status', $attraction->status) == 'published' ? 'selected' : '' }}>Published</option>
                                <option value="unpublished" {{ old('status', $attraction->status) == 'unpublished' ? 'selected' : '' }}>Unpublished</option>
                            </select>
                        </div>
                    ...
                    <div class="cat-tab-panel" data-panel="location">
                            ...
                            <div class="form-field">
                                ...
                                <select id="state_id" name="state_id" class="form-control-styled">
                                    ...
                                    @foreach($states as $state)
                                        <option value="{{ $state->id }}" {{ old('state_id', $attraction->state_id) == $state->id ? 'selected' : '' }}>
                                            {{ $state->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="form-field">
                            ...
                            <input type="text" id="map_location" name="map_location"
                                ...
                                value="{{ old('map_location', $attraction->map_location) }}"
                                placeholder="e.g. Kashmir, Jammu and Kashmir, India">
                    ...
                    <div class="cat-tab-panel" data-panel="gallery">
                        ...
                        <div id="gallery-repeater">
                            @foreach($galleries as $gallery)
                                <div class="gallery-row existing-gallery-row" data-id="{{ $gallery->id }}">
                                    <img src="{{ asset('storage/' . $gallery->image) }}" class="current-img-preview" alt="">
                                    <div class="form-row">
                                        <div class="form-field">
                                            <label>Title</label>
                                            <input type="text" name="gallery_titles_existing[{{ $gallery->id }}]"
                                                class="form-control-styled" value="{{ $gallery->title }}">
                                        </div>
                                        <div class="form-field">
                                            <label>Subtitle</label>
                                            <input type="text" name="gallery_subtitles_existing[{{ $gallery->id }}]"
                                                class="form-control-styled" value="{{ $gallery->subtitle }}">
                                        </div>
                                    </div>
                                    <button type="button" class="btn-secondary-dash remove-gallery-row"
                                        data-id="{{ $gallery->id }}">
                    ...
                    <div class="cat-tab-panel" data-panel="highlights">
                        ...
                        <div id="highlight-repeater">
                            @foreach($highlights as $highlight)
                                <div class="gallery-row existing-highlight-row" data-id="{{ $highlight->id }}">
                                    @if($highlight->icon)
                                        <img src="{{ asset('storage/' . $highlight->icon) }}" class="current-img-preview"
                                            alt="">
                                    @endif
                                    <div class="form-field">
                                        <label>Icon</label>
                                        <input type="file" name="highlight_icons_existing[{{ $highlight->id }}]"
                                            class="form-control-styled" accept="image/*">
                                        <div class="hint">Leave blank to keep the current icon</div>
                                    </div>
                                    <div class="form-field">
                                        <label>Title</label>
                                        <input type="text" name="highlight_titles_existing[{{ $highlight->id }}]"
                                            class="form-control-styled" value="{{ $highlight->title }}">
                                    </div>
                                    <div class="form-field">
                                        <label>Description</label>
                                        <textarea name="highlight_descriptions_existing[{{ $highlight->id }}]"
                                            class="form-control-styled" rows="2">{{ $highlight->description }}</textarea>
                                    </div>
                    ...
                    <div class="cat-tab-panel" data-panel="experiences">
                        ...
                        <div id="experience-repeater">
                            @foreach($experiences as $experience)
                                <div class="gallery-row existing-experience-row" data-id="{{ $experience->id }}">
                                    @if($experience->image)
                                        <img src="{{ asset('storage/' . $experience->image) }}" class="current-img-preview"
                                            alt="">
                                    @endif
                                    <div class="form-field">
                                        <label>Image</label>
                                        <input type="file" name="experience_images_existing[{{ $experience->id }}]"
                                            class="form-control-styled" accept="image/*">
                                        <div class="hint">Leave blank to keep the current image</div>
                                    </div>
                                    <div class="form-row" style="grid-template-columns: 1fr 1fr;">
                                        <div class="form-field">
                                            <label>Title</label>
                                            <input type="text" name="experience_titles_existing[{{ $experience->id }}]"
                                                class="form-control-styled" value="{{ $experience->title }}">
                                        </div>
                                        <div class="form-field">
                                            <label>Duration</label>
                                            <input type="text" name="experience_durations_existing[{{ $experience->id }}]"
                                                class="form-control-styled" value="{{ $experience->duration_text }}"
                                                placeholder="e.g. 1-2 Hours">
                                        </div>
                                    </div>
                                    <div class="form-field">
                                        <label>Description</label>
                                        <textarea name="experience_descriptions_existing[{{ $experience->id }}]"
                                            class="form-control-styled" rows="2">{{ $experience->description }}</textarea>
                                    </div>
                                    <button type="button" class="btn-secondary-dash remove-experience-row"
                    ...
                    <div class="cat-tab-panel" data-panel="places">
                        ...
                        <div id="place-repeater">
                            @foreach($places as $place)
                                <div class="gallery-row existing-place-row" data-id="{{ $place->id }}">
                                    @if($place->image)
                                        <img src="{{ asset('storage/' . $place->image) }}" class="current-img-preview" alt="">
                                    @endif
                                    <div class="form-field">
                                        <label>Image</label>
                                        <input type="file" name="place_images_existing[{{ $place->id }}]"
                                            class="form-control-styled" accept="image/*">
                                        <div class="hint">Leave blank to keep the current image</div>
                                    </div>
                                    <div class="form-row" style="grid-template-columns: 1fr 1fr;">
                                        <div class="form-field">
                                            <label>Title</label>
                                            <input type="text" name="place_titles_existing[{{ $place->id }}]"
                                                class="form-control-styled" value="{{ $place->title }}">
                                        </div>
                                        <div class="form-field">
                                            <label>Tag</label>
                                            <input type="text" name="place_tags_existing[{{ $place->id }}]"
                                                class="form-control-styled" value="{{ $place->tag }}"
                                                placeholder="e.g. Kashmir">
                                        </div>
                                    </div>
                                    <div class="form-field">
                                        <label>Description</label>
                                        <textarea name="place_descriptions_existing[{{ $place->id }}]"
                                            class="form-control-styled" rows="2">{{ $place->description }}</textarea>
                                    </div>
                                    <div class="form-field">
                                        <label>Button Text</label>
                                        <input type="text" name="place_button_texts_existing[{{ $place->id }}]"
                                            class="form-control-styled" value="{{ $place->button_text }}"
                                            placeholder="e.g. Explore Srinagar">
                                    </div>
                                    <button type="button" class="btn-secondary-dash remove-place-row"
                                        data-id="{{ $place->id }}">
                                        <i class="fa fa-trash"></i> Remove
                                    </button>
                                </div>
                    ...
                    <div class="cat-tab-panel" data-panel="offer">
                        ...
                        <div class="form-field">
                            ...
                            <input type="text" id="offer_badge_text" name="offer_badge_text" class="form-control-styled"
                                value="{{ old('offer_badge_text', $attraction->offer_badge_text ?? '') }}"
                        ...
                        <div class="form-field">
                            ...
                            <input type="text" id="offer_title" name="offer_title" class="form-control-styled"
                                value="{{ old('offer_title', $attraction->offer_title ?? '') }}"
                        ...
                        <div class="form-field">
                            ...
                            <textarea id="offer_description" name="offer_description" rows="3"
                                class="form-control-styled">{{ old('offer_description', $attraction->offer_description ?? '') }}</textarea>
                        </div>

                        <div class="form-field">
                            <label>Perks</label>
                            <div id="offer-perks-list">
                                @php $existingPerks = old('offer_perks', $attraction->offer_perks ?? ['', '', '']); @endphp
                                @foreach($existingPerks as $perk)
                                    <input type="text" name="offer_perks[]" class="form-control-styled"
                                        style="margin-bottom:8px;" value="{{ $perk }}"
                                        placeholder="e.g. Early Bird Discount up to 40% Off">
                                @endforeach
                            </div>
                        ...
                        <div class="form-field">
                            ...
                            @if(!empty($attraction->offer_image))
                                <img src="{{ asset('storage/' . $attraction->offer_image) }}" class="current-img-preview"
                                    alt="">
                            @endif
                        ...
                        <div class="form-row" style="grid-template-columns: 1fr 1fr;">
                            <div class="form-field">
                                ...
                                <input type="text" id="offer_button_text" name="offer_button_text"
                                    ...
                                    value="{{ old('offer_button_text', $attraction->offer_button_text ?? '') }}"
                            ...
                            <div class="form-field">
                                ...
                                <input type="text" id="offer_button_url" name="offer_button_url"
                                    class="form-control-styled"
                                    value="{{ old('offer_button_url', $attraction->offer_button_url ?? '') }}"
                                    placeholder="/listing">
                            </div>
                        </div>
                    ...
                    <div class="cat-tab-panel" data-panel="moreabout">
                        ...
                        <div class="form-field">
                            ...
                            <input type="text" id="about_more_title" name="about_more_title" class="form-control-styled"
                                value="{{ old('about_more_title', $attraction->about_more_title) }}"
                                placeholder="Defaults to the Attraction Name">
                        ...
                        <div class="form-field">
                            ...
                            <textarea id="about_more_content" name="about_more_content" rows="10"
                                class="ckeditor form-control-styled">{{ old('about_more_content', $attraction->about_more_content) }}</textarea>
                    ...
                    <div class="cat-tab-panel" data-panel="promo">
                        ...
                        <div class="form-field">
                            ...
                            <input type="text" id="promo_eyebrow" name="promo_eyebrow" class="form-control-styled"
                                value="{{ old('promo_eyebrow', $attraction->promo_eyebrow ?? 'Plan Your Trip') }}">
                        </div>

                        <div class="form-field">
                            <label for="promo_title">Title</label>
                            <input type="text" id="promo_title" name="promo_title" class="form-control-styled"
                                value="{{ old('promo_title', $attraction->promo_title) }}"
                                placeholder="Auto-filled: Ready to Explore [Name]?">
                        </div>

                        <div class="form-field">
                            <label for="promo_description">Description</label>
                            <textarea id="promo_description" name="promo_description" rows="3"
                                class="form-control-styled">{{ old('promo_description', $attraction->promo_description) }}</textarea>
                        </div>
                        ...
                        <div class="form-row" style="grid-template-columns: 1fr 1fr;">
                            <div class="form-field">
                                ...
                                <input type="text" id="promo_button_text" name="promo_button_text"
                                    class="form-control-styled"
                                    value="{{ old('promo_button_text', $attraction->promo_button_text) }}"
                            ...
                            <div class="form-field">
                                ...
                                <input type="text" id="promo_button_url" name="promo_button_url"
                                    class="form-control-styled"
                                    value="{{ old('promo_button_url', $attraction->promo_button_url) }}"
                                    placeholder="/listing">
                            </div>
                        </div>
                    ...
                    <div class="cat-tab-panel" data-panel="seo">
                        ...
                        <div class="form-field">
                            ...
                            <input type="text" id="meta_title" name="meta_title" class="form-control-styled"
                                value="{{ old('meta_title', $attraction->meta_title) }}">
                        </div>

                        <div class="form-field">
                            <label for="meta_description">Meta Description</label>
                            <textarea id="meta_description" name="meta_description" rows="4"
                                class="form-control-styled">{{ old('meta_description', $attraction->meta_description) }}</textarea>
                        </div>
                        ...
                        <div class="form-field">
                            ...
                            <input type="text" id="og_title" name="og_title"
                                ...
                                value="{{ old('og_title', $attraction->og_title) }}">
                            @error('og_title')
                            <div class="form-error">{{ $message }}</div>@enderror
                        </div>
                        ...
                        <div class="form-field">
                            ...
                            <textarea id="og_description" name="og_description" rows="3"
                                class="form-control-styled @error('og_description') is-invalid @enderror">{{ old('og_description', $attraction->og_description) }}</textarea>
                        ...
                        <div class="form-field">
                            ...
                            @if($attraction->og_image)
                                <img src="{{ asset('storage/' . $attraction->og_image) }}" class="current-img-preview"
                                    alt="">
                            @endif
                    ...
                    <div class="form-actions">
                        <button type="submit" class="btn-primary-dash">
                            <i class="fa fa-check"></i> Update Attraction
                        </button>
...
{{-- Existing itinerary data handed to JS for pre-population --}}
<script id="existing-itineraries-data" type="application/json">
    {!! json_encode($itineraries->map(fn($i) => [
    'days' => $i->days,
    'title' => $i->title,
    'is_popular' => $i->is_popular,
    'stops' => $i->stops->pluck('stop_name'),
])) !!}
</script>
...
{{-- Existing travel-info data handed to JS for pre-population --}}
<script id="existing-seasons-data" type="application/json">
    {!! json_encode($seasons->map(fn($s) => [
    'months' => $s->months,
    'title' => $s->title,
...
<script id="existing-transports-data" type="application/json">
    {!! json_encode($transports->map(fn($t) => [
...
<script id="existing-budget-tiers-data" type="application/json">
    {!! json_encode($budgetTiers->map(fn($b) => [
...
<script id="existing-carry-groups-data" type="application/json">
    {!! json_encode($carryGroups->map(fn($c) => [
    'title' => $c->title,
    'items' => $c->items ?? [],
])) !!}
</script>
...
<script>
    ...
    document.querySelectorAll('.remove-gallery-row').forEach(function (btn) {
        btn.addEventListener('click', function () {
            ...
            this.closest('.existing-gallery-row').remove();
        });
    });
    ...
    // Pre-populate on edit page (no-op on create page since the script tag won't exist)
    const existingItinerariesEl = document.getElementById('existing-itineraries-data');
    if (existingItinerariesEl) {
        const existingItineraries = JSON.parse(existingItinerariesEl.textContent);
        existingItineraries.forEach(data => addItineraryRow(data));
    }
    ...
    const existingSeasonsEl = document.getElementById('existing-seasons-data');
    if (existingSeasonsEl) {
        JSON.parse(existingSeasonsEl.textContent).forEach(data => addSeasonRow(data));
    }
    ...
    const existingTransportsEl = document.getElementById('existing-transports-data');
    if (existingTransportsEl) {
        JSON.parse(existingTransportsEl.textContent).forEach(data => addTransportRow(data));
    }
    ...
    const existingBudgetTiersEl = document.getElementById('existing-budget-tiers-data');
    if (existingBudgetTiersEl) {
        JSON.parse(existingBudgetTiersEl.textContent).forEach(data => addBudgetRow(data));
    }
    ...
    const existingCarryGroupsEl = document.getElementById('existing-carry-groups-data');
    if (existingCarryGroupsEl) {
        JSON.parse(existingCarryGroupsEl.textContent).forEach(data => addCarryRow(data));
    }
    ...
    const existingFaqsEl = document.getElementById('existing-faqs-data');
    if (existingFaqsEl) {
        JSON.parse(existingFaqsEl.textContent).forEach(data => addFaqRow(data));
    }
</script>

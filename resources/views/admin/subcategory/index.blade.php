{{-- resources/views/admin/subcategory/index.blade.php --}}
                ...
                <table class="dest-table">
                    ...
                    <tbody>
                            ...
                            <tr>
                                <td>
                                    @if($subCategory->banner_image_one)
                                        <img src="{{ asset($subCategory->banner_image_one) }}" class="dest-thumb" alt="{{ $subCategory->name }}">
                                    @else
                                ...
                                <td>
                                    <div class="dest-name">{{ $subCategory->name }}</div>
                                    <div class="dest-location">{{ Str::limit($subCategory->intro_text, 50) }}</div>
                                </td>
                                <td>
                                    @if($subCategory->category)
                                        <span class="badge-cat">{{ $subCategory->category->name }}</span>
                                    @else
                                        —
                                    @endif
                                </td>
                                <td>{{ $subCategory->h1 ?? '—' }}</td>
                                <td>
                                    <span class="badge-status badge-{{ $subCategory->status }}">
                        ...
                        @empty
                            <tr>
                                <td colspan="6">
                                    <div class="empty-state">No sub categories added yet.</div>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>

...
<div class="main-section">
    ...
    .star-select label { display: inline-flex; align-items: center; gap: 4px; font-size: 13px; font-weight: 500; border: 1px solid var(--border); border-radius: var(--radius-sm); padding: 8px 12px; cursor: pointer; }
    .star-select input { margin: 0; }
    </style>
                    ...
                    <div style="padding:24px;">
                            ...
                            <div class="form-field">
                                ...
                                <select id="reviewable_type" name="reviewable_type"
                                    ...
                                    @foreach($types as $key => $class)
                                        <option value="{{ $key }}" {{ old('reviewable_type') == $key ? 'selected' : '' }}>
                                            {{ ucwords(str_replace('_', ' ', $key)) }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('reviewable_type')<div class="form-error">{{ $message }}</div>@enderror
                            </div>

                            <div class="form-field">
                                <label for="reviewable_id">Select Item</label>
                                <select id="reviewable_id" name="reviewable_id"
                                    class="form-control-styled @error('reviewable_id') is-invalid @enderror" disabled required>
                                    <option value="">Select Type First</option>
                                </select>
                                @error('reviewable_id')<div class="form-error">{{ $message }}</div>@enderror
                            </div>
                        </div>
                        ...
                        <div class="form-row">
                            <div class="form-field">
                                ...
                                <input type="text" id="full_name" name="full_name"
                                    ...
                                    value="{{ old('full_name') }}" placeholder="e.g. Sanjeev Ahuja" required>
                            ...
                            <div class="form-field">
                                ...
                                <input type="text" id="designation" name="designation"
                                    class="form-control-styled" value="{{ old('designation') }}"
                        ...
                        <div class="form-field">
                            ...
                            <textarea id="review" name="review" rows="4"
                                ...
                                placeholder="What did they say about the experience?" required>{{ old('review') }}</textarea>
                            @error('review')<div class="form-error">{{ $message }}</div>@enderror
                        </div>

                        <div class="form-row">
                            <div class="form-field">
                                <label for="photo">Reviewer Photo (optional)</label>
                                <input type="file" id="photo" name="photo" class="form-control-styled" accept="image/*">
                                <div class="hint">If left empty, initials will be shown instead</div>
                            </div>
    ...
    typeSelect.addEventListener('change', function () {
        ...
        fetch(`{{ url('admin/reviews/entities') }}/${this.value}`)
            ...
            .then(items => {
                ...
                items.forEach(item => {
                    entitySelect.innerHTML += `<option value="${item.id}">${item.name}</option>`;
                });
